...
Looking at the F-statistic for all models were large and achieved a statistically significant p-value than than a default alpha of .05; thus for all three models the Null hypothesis that is rejected that 0s for our regression coefficients would fit the data better.

From the exploratory data analysis we saw that many of the predictor variables were correlated with each other which could potentially be an issue from a multicolinearity, that can lead to unstable regression fits. From the previous section we used VIF (Variable Inflation Factor) to gauge better models by preferring VIF values less than 5. 

Additionally, the model analysis showed that 'nox' (nitrous oxide emissions), and 'rad' (highway accessibility) were consistently greater predictors for each of the model's produced.
...
Based on these analyses, Model 1 and Model 2 both performed marginally better than Model 4 but model 2 performed besed when looking at the AUC and f-statistic. This would be the selected model of choice. This model also produced the greatest R^2, However, one thing to note is that there is greater deviance under the third model relative to the others. 
