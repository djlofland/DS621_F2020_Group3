...
Bimodal features are extremely interesting in classification tasks, as the could indicate overlapping but seperate distributions for each class, which could provide powerful predictive power in a model.
...
Finally, we wanted to plot scatter plots of each variable versus the target variable to get an idea of the relationship between them. 
...
Although it does not appear that any 1 feature clearly distinguishes the two target values, there are many features that appear to have some significant correlation with the target, including `lstat`, `age`, `dis`, `zn`, and `nox`. This indicates that our data may be predictive and the model building process is likely to model an actual relationship rather than just overfitting to the quirks of the training data.
...
It appears that `nox`, `age`, `rad`, `tax`, and `indus` have the highest correlation (positive) with `target`; this appears to be somewhat counterintuitive, as older buildings, more residential neighborhoods, and higher property tax values appear to lead to higher crime, not what was expected. The strong positive correlation with nitrous oxide is also interesting and unexpected. The other variables have weak or slightly negative correlation, which implies they have less predictive power.

`dis`, `zn`, and `medv` all have a negative correlation to `target`, indicating that more distant areas with large lot sizes and high median home values are less likely to be high crime, all of which seems sensible.
...
First we decided to split our datasets (both original and transformed) into test/train split (20/80).

```{r}

# origdfTrain <- createDataPartition(df$target, p=0.8, list=FALSE)
# origdftraining <- df[origdfTrain,]
# origdftesting <- df[-origdfTrain,]
set.seed(123456)
...
Using our training dataset, we decided to run a binary logistic regression model (Model #1) that included all non-transformed features that we hadn't removed following our data cleaning process mentioned above. 

```{r}
model1 <- glm(target ~ zn + indus + chas + nox + rm + age + dis + rad + tax + ptratio + lstat + medv, data = cleaneddftraining , family = binomial)
summary(model1)
```
We also calculated VIF scores to measure effects of collinearity as well as variable importance:
```{r}
...
```
Our next model we used a Stepwise AIC on model 1 (our model with non-transformed features) to pick which features were most relevant.
...
In our final model we used a Stepwise AIC on model 2 (our model with transformed features) to pick which features were most relevant.
...
This is a trend we saw throughout the four models that we built, although Models #3 and #4 were able to adjust for this better than our first two models -- we can likely attribute this phenomenon to multicolinearity. Since we noticed in our initial data exploration that many variables in the dataset were highly correlated with one another (i.e. `medv` and `zn`, `nox` and `indus`, `tax` and `rad`), this phenomenon likely is increasing the variance of the coefficient estimates, making them difficult to interpret (and in some cases such as the features listed above, they are switching the signs). This was also supported by our Variance Inflation Factor (VIF) tests, which showed high values for features such as [`medv` and `rn`]. In our final models (Models #3 and #4), we made sure to keep this in mind in order to get a better handle on our coefficients and reduce multicolinearity -- mainly, we removed certain variables that had high VIF scores through our stepwise selection process. 
...
The above table summarizes the accuracy, F-statitics, deviance, R^2 (McFaden) and AIC of all of our models. 

Overall--Model 4 had the highest accuracy and F statistic and AIC Model 2 had the lowest deviance and highest R^2.

Model 1 and Model 3 had near identical accuracy, F statistic, deviance and R^2--and differed only in AIC, with model 3 having a slightly lower (172.76 vs 175.99). Since both models are based on the raw untransformed data, and model 3 was just an optimized AIC, this makes sense, and is probably due to the removal of a variable that has no predictive power. In relation to the other models, none of the untransformed models achieved any of the highest metrics, which we think is due to the lack of transformation/processing the data.


Model 1 - This model is based on the all non-transformed variables. Results for this model provided an F1 value of 0.94, and an ROC AUC of 0.93. 

Model 2 - This model is based on a subset of significant predictors on an un-transformed dataset. Results for this model provided an F1 value of 0.94, and an ROC AUC of 0.95. 


Model 3 - This model is based on a subset of significant predictors on an transformed dataset. Results for this model provided an F1 value of 0.93, and an ROC AUC of 0.93. 

Model 4 - This model is based on a subset of significant predictors on an transformed dataset. Results for this model provided an F1 value of 0.92, and an ROC AUC of 0.92. 

Model 2 and Model 4 both had lower AIC than Model 1 and Model 3, however Model 2 has a higher AIC than Model 4--once again this makes sense since Model 4 is an optimized AIC of model 2. Model 2 the lowest accuracy and F statistic of our group of models but highest R^2 and lowest deviance. 
...
Based on all of our models we selected Model 4--which had the highest F-statistic and accuracy. Its deviance and R^2 was 2nd best (to model 2), and more importantly it had the lowest AIC, which via occams razor, shows a simpler model is better.

## Predictions

We apply model 4 to the held out evaluation set to predict the targets for these 40 instances. We have saved these predictions in the .csv file eval_predictions.
