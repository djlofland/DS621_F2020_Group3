...
We see that there are some columns with significant missing values. There are also outliers present in the data.
...
gather_clean_df <- clean_df %>% gather(key = 'variable', value = 'value', -INDEX, -TARGET_WINS)
```
...
col_summary <- gather_clean_df %>% group_by(variable) %>% summarise(count = n(), mean = mean(value), med = median(value), sd = sd(value), min = min(value), max = max(value)) %>% as.data.frame()
...
featurePlot(clean_df[,3:ncol(clean_df)], clean_df[,2], pch = 20)
```
...
```{r, fig.height = 10, fig.width = 8}
clean_df %>% ggpairs(columns = 2:ncol(clean_df))
```
...
```{r}
correlation = cor(clean_df[,3:ncol(clean_df)], use = 'pairwise.complete.obs')
