...
no_outlier_df$TEAM_PITCHING_SO <- ifelse(no_outlier_df$TEAM_PITCHING_SO > 4000, NA, no_outlier_df$TEAM_PITCHING_SO)

# 5000 hits is an average of 30 hits allowed per game, which is also ridiculous.
no_outlier_df$TEAM_PITCHING_H <- ifelse(no_outlier_df$TEAM_PITCHING_H > 5000, NA, no_outlier_df$TEAM_PITCHING_H)

# 2000 walks is an average of 13 walks per game which is unlikely.
no_outlier_df$TEAM_PITCHING_BB <- ifelse(no_outlier_df$TEAM_PITCHING_BB > 2000, NA, no_outlier_df$TEAM_PITCHING_BB)
...
gather_clean_df %>% ggplot(aes(x = value, y = TARGET_WINS)) + geom_point() + facet_wrap(~variable, scales = 'free')
