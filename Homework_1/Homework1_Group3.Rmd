...
df_temp$BATTING_3B_transform <- log(BoxCox(clean_df$BATTING_3B, batting3b_lambda))
...
VarImpPlot<-function(LINEARMODEL){
  varImp(LINEARMODEL) %>% as.data.frame() %>% 
    ...
    scale_fill_gradient()+
    labs(title = "LM Variable Importance",
         x = "Parameter",
...
```
Comparing our above model to the reduced model below we can see we maintained the important predictors and the relative strength remains comparable.
```{r echo=FALSE}
VarImpPlot(mult_lm_final)
```
