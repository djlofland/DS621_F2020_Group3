...
transformative_cleaning<-function(DF, TEMP){
  return(
    ...
    DF %>% mutate_at(c("AGE","BLUEBOOK","CAR_AGE","HOME_VAL","INCOME","TIF","TRAVTIME"), 
      ...
      mutate_if(is.numeric, list(~na_if(., -Inf))) %>% 
      rename("AGE_transform" = "AGE", "BLUEBOOK_transform" = "BLUEBOOK", "CAR_AGE_transform" = "CAR_AGE", "HOME_VAL_transform" = "HOME_VAL", "INCOME_transform" = "INCOME", "TIF_transform" = "TIF", "TRAVTIME_transform" = "TRAVTIME", "TARGET_AMT_transform" = "TARGET_AMT", "OLDCLAIM_transform" = "OLDCLAIM") %>% 
...
df_temp <- df %>% select("AGE", "BLUEBOOK", "CAR_AGE", "HOME_VAL", "INCOME", "TIF", "TRAVTIME", "TARGET_AMT", "OLDCLAIM")
# Build clean dataframe with transformation
clean_df <- transformative_cleaning(df, df_temp)
...
**First linear regression model, no transformations, all nominal predictors included**
```{r}
...
model1 <- lm(TARGET_AMT ~ AGE + BLUEBOOK + CAR_AGE + CLM_FREQ + HOME_VAL + HOMEKIDS + INCOME + KIDSDRIV + MVR_PTS + OLDCLAIM + TIF + TRAVTIME + YOJ + CAR_TYPE + CAR_USE + EDUCATION + JOB + MSTATUS + PARENT1 + RED_CAR + REVOKED + SEX + URBANICITY, df_train)
...
#                          scope = list(upper = model1, lower = ~ 1),
#                          scale = 0, trace = FALSE)
