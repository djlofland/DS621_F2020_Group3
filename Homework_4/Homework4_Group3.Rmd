...
df_temp <- df %>% select("BLUEBOOK", "CAR_AGE", "HOME_VAL", "INCOME", "TIF", "TRAVTIME", "TARGET_AMT", "OLDCLAIM")
...
model3recipe<- 
  ...
  #remove all non numeric based variables
  step_rm(all_nominal()) %>%
  step_rm(c(AGE,YOJ,CLM_FREQ,INCOME_transform)) %>% 
...
model3<-lm_spec %>% 
  fit(TARGET_AMT_transform ~ .,data = m3train)
...
grid.arrange(linVP1,linVP2,linVP3,logVP1,logVP2,logVP3,ncol = 3)
